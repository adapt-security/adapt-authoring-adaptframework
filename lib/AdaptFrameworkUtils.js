...
class AdaptFrameworkUtils {
  ...
  static async spawnPromise (cmd, options = {}) {
    AdaptFrameworkUtils.log('debug', 'SPAWN', cmd)
    const framework = await App.instance.waitForModule('adaptframework')
    return new Promise((resolve, reject) => {
      const cwd = framework.path
      const task = spawn(cmd, { cwd })
      let output = ''
      let error = ''
      task.stdout.on('data', data => output += data)
      task.stderr.on('data', data => error += data)
      task.on('close', code => {
        if(code !== 0) {
          if(options.logError !== false) AdaptFrameworkUtils.log('debug', 'SPAWN_STDERR', task, stderr)
          return reject(framework.app.errors.SPAWN.setData({ stderr: error }))
        }
        if(options.logOutput !== false) AdaptFrameworkUtils.log('debug', 'SPAWN_STDOUT', task, stdout)
        resolve(output)
      })
    })
  }
