...
class AdaptFrameworkUtils {
  ...
  static logDir (label, dir) {
    ...
    this.log('debug', 'DIR', label, resolved)
    try {
      this.log('debug', 'DIR_MODE', label, fs.statSync(resolved).mode)
    } catch (e) {
      this.log('warn', `failed to stat dir ${resolved}`)
    }
  }

  static logMemory () {
    this.log('debug', 'MEMORY', Object.entries(process.memoryUsage()).reduce((m, [k, v]) => Object.assign(m, { [k]: bytes.parse(v) }), {}))
  }
  ...
  static async postHandler (req, res, next) {
    ...
    this.log('info', `running ${action} for course '${courseId}' initiated by ${userId}`)
    try {
      ...
      this.log('info', `finished ${action} for course '${courseId}' in ${duration} seconds`)
      ...
      this.log('error', `failed to ${action} course '${courseId}'`)
      return next(e)
    }
  }
  ...
  static async postUpdateHandler (req, res, next) {
    try {
      this.log('info', 'running framework update')
      const framework = await App.instance.waitForModule('adaptframework')
