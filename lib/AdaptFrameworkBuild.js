...
class AdaptFrameworkBuild {
  ...
  async build () {
    ...
    if (this.isPreview && !contentOnly) {
      const prebuiltCacheRoot = path.join(framework.getConfig('buildDir'), 'prebuilt-cache')
      ...
      if (await hasCachedBuild(prebuiltCacheRoot, pluginHash, theme, menu)) {
        await restoreFromCache(prebuiltCacheRoot, pluginHash, theme, menu, this.buildDir)
    ...
    if (this.isPreview && !contentOnly) {
      ...
      const menu = this.courseData.config.data._menu
      try {
        await populateCache(this.buildDir, prebuiltCacheRoot, pluginHash, theme, menu)
      } catch (e) {
  ...
  async applySchemaDefaults () {
    ...
    const [courseSchema, configSchema] = await Promise.all([
      jsonschema.getSchema('course'),
      jsonschema.getSchema('config')
    ])
