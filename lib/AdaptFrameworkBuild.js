...
class AdaptFrameworkBuild {
  ...
  async determineLanguages () {
    ...
    const courses = await mongodb.find('content', { _type: 'course', _courseId: this.courseId })
    const masterCourse = courses.find(c => c._id.toString() === this.courseId)

    this.masterLanguage = masterCourse._lang
    // ensure masterLanguage is first in array
    this.languages = _.uniq([this.masterLanguage].concat(_.map(courses, '_lang')))
  }
  ...
  async build () {
    ...
    this.assetsDir = path.join(this.courseDir, this.masterLanguage, 'assets')
  ...
  async loadCourseData () {
    ...
    // course data for the master language will be first in the list
