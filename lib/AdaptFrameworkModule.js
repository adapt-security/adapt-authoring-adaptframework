...
import { runCliCommand, readFrameworkPluginVersions, migrateExistingCourses, computePluginHash, hasSharedCache, invalidateCache, prebuildSharedCache } from './utils.js'
...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async init () {
    ...
    if (this.getConfig('prebuildSharedCache')) {
      const cacheRoot = path.join(this.getConfig('buildDir'), 'prebuilt-cache')
      this.getPluginHash()
        .then(async pluginHash => {
          if (!(await hasSharedCache(cacheRoot, pluginHash))) this.prebuildSharedCache()
        })
        .catch(e => this.log('warn', `failed to check prebuilt cache on boot: ${e.message}`))
    }
  ...
  async invalidatePrebuiltCache () {
    ...
    if (this.getConfig('prebuildSharedCache')) {
      this.prebuildSharedCache()
    }
  }

  /**
   * Eagerly rebuilds the shared prebuilt cache in the background.
   * Safe to call multiple times — if a build is already in progress,
  ...
  prebuildSharedCache () {
    ...
    this._eagerBuildPromise = prebuildSharedCache({
      buildDir: this.getConfig('buildDir'),
      frameworkDir: this.path
    }).catch(e => {
      this.log('warn', `eager shared cache build failed: ${e.message}`)
