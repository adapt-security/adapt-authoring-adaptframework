...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async init () {
    ...
    this.postInstallHook.tap(this.installFrameworkModules())
    /**
     * Invoked after a framework update
     * @type {Hook}
     */
    this.postUpdateHook = new Hook()
    this.postUpdateHook.tap(this.installFrameworkModules())
  ...
  async installFrameworkModules () {
    try {
      exec('npx install', { cwd: this.path }, (error, stdout, stderr) => {
