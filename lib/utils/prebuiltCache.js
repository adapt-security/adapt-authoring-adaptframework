...
/** Shared artifact paths relative to build dir (JS bundle, templates, plugin assets) */
const SHARED_GLOBS = [
  'adapt/js',
  'templates.js',
  'assets',
...
/** CSS artifact paths relative to build dir */
const CSS_PATHS = ['adapt/css']
...
export async function populateCache (buildOutputDir, cacheRoot, pluginHash, theme, menu) {
  ...
  try {
    ...
    for (const rel of SHARED_GLOBS) {
      const src = path.join(buildOutputDir, rel)
      const dest = path.join(tmpShared, rel)
      try {
        ...
        // Skip missing optional artifacts
      }
    }
    ...
    for (const rel of CSS_PATHS) {
      const src = path.join(buildOutputDir, rel)
      const dest = path.join(tmpCss, rel)
      try {
        await fs.cp(src, dest, { recursive: true })
      } catch (e) {
        if (e.code !== 'ENOENT') throw e
