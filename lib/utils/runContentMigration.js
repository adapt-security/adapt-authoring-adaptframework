...
import { createErrorCapturingLogger } from './createErrorCapturingLogger.js'
...
export async function runContentMigration ({ content, fromPlugins, toPlugins, scripts, cachePath }) {
  ...
  try {
    ...
    // adapt-migrations swallows failed migration steps (it logs the error and
    // rolls back rather than throwing), so capture error-level logs during the
    // run and surface them as a thrown error. Remove when adapt-migrations
    // propagates the error itself and revert to `await migrate({ journal, logger })`.
    const capturingLogger = createErrorCapturingLogger(logger)
    await migrate({ journal, logger: capturingLogger })
    if (capturingLogger.errors.length) {
      throw new Error(`adapt-migrations reported ${capturingLogger.errors.length} error(s) during migration: ${capturingLogger.errors.join('; ')}`)
    }
