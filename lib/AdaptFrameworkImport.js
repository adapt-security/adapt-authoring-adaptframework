...
class AdaptFrameworkImport {
  ...
  // Run migration grunt tasks
  async runGruntTask (task) {
    return new Promise((resolve, reject) => {
      const cmd = `grunt ${task}`;
      exec(cmd, { cwd: this.unzipPath }, (error, stdout, stderr, logger) => {
        if (error) {
          const e = new Error(`Error execSyncuting grunt task ${cmd}`);
          return reject(e)
        }
        resolve(stdout)
      })
  ...
  // Move migration scripts from tool plugin to import plugin
  async addMigrationScripts () {
    // ONLY NEEDED FOR TESTING, WILL BE RUN AFTER this.installedPlugins IS POPULATED WHEN NOT RUN ON DRYRUN
    ...
    Object.keys(this.usedContentPlugins).forEach(p => {
      ...
      if (semver.lte(installedVersion, importVersion)) {
    ...
    if (pluginsToUpdate.length) {
      ...
      pluginsToUpdate.forEach(async p => {
        console.debug(`Adding migration scripts for ${p}`);
        // get framework route
        const frameworkPath = this.framework.path;
        const [pluginMigrationPaths] = await glob(`${this.framework.path}/**/**/${p}/migrations`);
        // return early if no migration scripts found
        ...
        try {
          ...
          if (!destinationPath) {
            ...
            // destinationPath = path.join(pluginPath, 'migrations');
          }

          // copy migration scripts from tool plugin to import plugin
  ...
  async migrateCourseData () {
    ...
    try {
      execSync('npm install adapt-migrations', { cwd: this.unzipPath });
