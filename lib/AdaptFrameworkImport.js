...
class AdaptFrameworkImport {
  ...
  async import () {
    ...
    try {
      ...
      const error = {
        error: e.meta.description,
        ...(courseTitle && { courseTitle }),
        ...(e.data?.data && { data: e.data.data })
      }
      throw App.instance.errors.COURSE_DATA.setData({ data: JSON.stringify(error) })
    }
  ...
  async loadCourseData () {
    ...
    try {
      ...
      const error = { description: App.instance.errors.FW_IMPORT_MISSING_CONFIG.meta.description }
      throw App.instance.errors.COURSE_DATA.setData({ data: JSON.stringify(error) })
    }
  ...
  async importTags () {
    ...
    const existingTagMap = (await tags.find()).reduce((memo, tag) => Object.assign(memo, { [tag.title]: tag._id }), {})
    const newTags = this.tags.slice()
    // add asset tags
    this.assetData.forEach(a => {
      if (a.tags) newTags.push(...a.tags.filter(t => !existingTagMap[t.title]))
    })
    if (this.settings.isDryRun) {
      this.statusReport.info.push({ code: 'TAGS_IMPORTED', data: { count: newTags.length } })
      return
    }
    await Promise.all(newTags.map(async t => {
      if (!existingTagMap[t.title]) {
        const { _id } = await tags.insert({ title: t.title })
        existingTagMap[t] = _id
      }
    }))
    this.tags = this.tags.map(t => existingTagMap[t].toString())
    // convert assetData tags to expected format
    this.assetData.forEach(a => {
      a.tags = a.tags?.map(t => existingTagMap[t.title]?.toString())
    })
  }
  ...
  getSortedData (courseObject, courseContent) {
    ...
    while (sortedCount < cos.length) {
      ...
      else throw App.instance.errors.FW_IMPORT_CONTENT_FAILED // level has no children, so something's gone wrong
    }
