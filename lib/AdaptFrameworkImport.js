...
class AdaptFrameworkImport {
    ...
    try {
      ...
      if (error) {
        ...
        if (this.newTagIds.length > 0) {
          const tags = await App.instance.waitForModule('tags')
          tasks.push(
            Promise.all(this.newTagIds.map(id => tags.delete({ _id: id })))
          )
        }
        let _courseId
        try {
  ...
  async restoreUpdatedPlugins () {
    // Batch lookup all current plugins to avoid multiple database queries
    const pluginNames = Object.keys(this.updatedContentPlugins)
    if (pluginNames.length === 0) return Promise.resolve()

    const currentPlugins = await this.contentplugin.find({ name: { $in: pluginNames } })
    const pluginMap = currentPlugins.reduce((map, p) => {
      map[p.name] = p
      return map
    }, {})
    ...
    for (const [pluginName, originalMetadata] of Object.entries(this.updatedContentPlugins)) {
      try {
        const currentPlugin = pluginMap[pluginName]
        if (currentPlugin) {
          // Note: Local plugins (isLocalInstall=true) cannot be fully restored to their
          // original cached version. They will be uninstalled, leaving the system without
          // the plugin until it's manually reinstalled.
          if (originalMetadata.isLocalInstall) {
            FWUtils.log('warn', `rolling back locally installed plugin '${pluginName}' (was version ${originalMetadata.version}) - original cached version cannot be automatically restored`)
          } else {
            FWUtils.log('info', `rolling back updated plugin '${pluginName}' (was version ${originalMetadata.version})`)
          }
          restoreTasks.push(
            this.contentplugin.uninstallPlugin(currentPlugin._id)
              .catch(e => FWUtils.log('error', `failed to uninstall updated plugin '${pluginName}', ${e.message}`))
          )
        }
      } catch (e) {
        FWUtils.log('error', `failed to rollback plugin '${pluginName}', ${e.message}`)
      }
    }
