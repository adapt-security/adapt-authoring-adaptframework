...
class AdaptFrameworkImport {
  ...
  async loadCourseData () {
    ...
    this.statusReport.warn.push({ code: 'CONTENT_IMPORTED', data: AdaptFrameworkUtils.getImportContentCounts(this.contentJson) })
  ...
  async runGruntTask (task) {
    return new Promise((resolve, reject) => {
      exec(`npx grunt ${task}`, { cwd: this.unzipPath }, (error, stdout) => {
        this.framework.log('debug', 'GRUNT', task, stdout)
  ...
  async migrateCourseData () {
    try {
      execSync('npm install adaptlearning/adapt-migrations#issue/42', { cwd: this.unzipPath })
