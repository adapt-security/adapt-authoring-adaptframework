...
class AdaptFrameworkImport {
  ...
  async restoreUpdatedPlugins () {
    ...
    for (const [pluginName, originalMetadata] of Object.entries(this.updatedContentPlugins)) {
      try {
        FWUtils.log('warn', `rollback: uninstalling updated plugin '${pluginName}' (was version ${originalMetadata.version})`)
        // Find the current plugin and uninstall it
        const currentPlugin = await this.contentplugin.find({ name: pluginName })
        if (currentPlugin.length > 0) {
          restoreTasks.push(
            this.contentplugin.uninstallPlugin(currentPlugin[0]._id)
